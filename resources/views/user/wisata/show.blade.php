...
$ref = request('ref');
if ($ref === 'rekomendasi') {
$backUrl = route('saw.recommendations.index');
$backLabel = 'Rekomendasi';
} elseif ($ref === 'ranking') {
$backUrl = route('saw.results.index');
$backLabel = 'Ranking Wisata';
} else {
$backUrl = route('wisata.catalog');
$backLabel = 'Daftar Wisata';
}
@endphp
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ $backUrl }}" class="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-4 rounded">
                ← Kembali
            </a>
        </div>
    </x-slot>
...
<script>
    ...
    L.marker([{{ $wisata->latitude }}, {{ $wisata->longitude }}])
        .addTo(map)
        .bindPopup(
            '<b>{{ addslashes($wisata->name) }}</b><br>{{ addslashes($wisata->location) }}'
        )
        .openPopup();
